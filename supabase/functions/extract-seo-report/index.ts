...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an SEO report data extractor. Given a PDF SEO report, extract the following metrics as JSON. Be precise and pull numbers directly from the report.

Return a JSON object with these fields:
- month: string in "YYYY-MM" format (derive from the report date)
- domain_authority: number (the DA score, look in backlink profile or KPI section)
...
- top_keywords: array of objects with { keyword: string, position: number, change: string }
  - For position, use the average/current position number
        ...
        messages: [
          { role: "system", content: systemPrompt },
          {
            ...
            content: [
              {
                type: "text",
                text: "Extract SEO metrics from this PDF report. Return only the JSON object.",
              },
              {
                type: "image_url",
                image_url: {
